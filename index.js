...
function CemberinCevresi(yaricap) {
  return CemberinCevresi = 2 * pi * yaricap;
};
...
// 3f çözümü
tekraredensayilar = sayi.toSring();
let sayacObjesi = {};

sayilar.forEach((sayi)=>{
  let anahtar = sayilar.toString();
  if (sayacObjesi[anahtar]){
    sayacObjesi[anahtar]++;
  }else{
    sayacObjesi[anahtar] = 1;
  }
});
for (let sayi in sayacObjesi) {
  if (sayacObjesi[sayi] > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${sayacObjesi[sayi]} kere tekrar edilmiştir`);
  }
};
